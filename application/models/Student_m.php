...
class Student_m extends CI_Model
{
	...
	public function getAssignmentData($major) {
		$query = "select a.*, u.first_name, u.last_name from assignment a left join users u on u.id = a.teacher_id where major = $major and status = 1";
		return $this->db->query($query);
	}
}
